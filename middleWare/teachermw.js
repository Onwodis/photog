...
const authteacher = async (req, res) => {
  ...
  if (name && username && email && pwrd && cpwrd&&phone) {
    if (pwrd == cpwrd) {
      ...
      if (pwrd.length > 5) {
        const ifEmail = await teacherModel.findOne({ email: email });
        if (!ifEmail) {
          console.log('line 41');
          if (username.length > 3) {
            const ifaddteacher = await schoolModel.findOne({
              signteacher: href,
            });
            if (ifaddteacher) {
              const ifUsername = await teacherModel.findOne({
                username: username,
              });
              if (!ifUsername) {
...
module.exports = { authteacher, vpin };
